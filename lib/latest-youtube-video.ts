...
const CHANNEL_ID = "UCl05Wu_P5GYQvzFppZ3BzNw";
...
async function fetchYouTubeFeed(
  ...
  for (
    ...
    try {
      ...
      console.warn(
        `[YouTube] Tentativo ${attempt}/${FETCH_ATTEMPTS} fallito.`
      );
...
export async function getLatestYouTubeVideo(): Promise<LatestYouTubeVideo | null> {
  ...
  try {
    ...
    console.error(
      "[YouTube] Impossibile recuperare l'ultimo video del canale. Il sito continuerà senza il video.",
      error
    );
